...
_ATHENA_OPTIMIZE_USD_PER_TB = Decimal("5")  # one-time compaction cost
...
class AthenaCostEstimator:
    ...
    def estimate_cost(
        ...
        # One-time OPTIMIZE compaction cost
        total_stored_tb = Decimal(str(profile.total_stored_gb)) / Decimal("1024")
        optimize_cost = total_stored_tb * _ATHENA_OPTIMIZE_USD_PER_TB
        ...
        return EngineCostEstimate(
            ...
            source_note=(
                ...
                f"One-time OPTIMIZE estimate is an upper bound — based on uncompressed BigQuery logical size; "
